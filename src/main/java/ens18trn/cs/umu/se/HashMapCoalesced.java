...
public class HashMapCoalesced<K,V> extends HashMap<K, V> {
    /**
     ...
     */
    public HashMapCoalesced(int initialCapacity, float loadFactor) {
     ...
     * @throws IllegalArgumentException if the initial capacity is negative.
     */
    public HashMapCoalesced(int initialCapacity) {
    ...
    /**
     * Implements the Coalesced hashing algorithm for Lookup and getting the value
    ...
    final Node<K,V> getNode(Object key) {
        ...
        int m, hash, i;
        K k;
        Node<K, V> node1 = coalescedHashingLookup(key);
        if (node1 != null) return node1;
        return null;
    }

    private Node<K, V> coalescedHashingLookup(Object key){
        Node<K, V>[] tab;
        int i;
        int m;
        int hash = hash(key);
        ...
        if ((tab = table) != null && (m = tab.length) > 0 && (node = tab[i = getIndex(hash, m)]) != null) {
            ...
            if (node.hash == hash && ((k = node.key) == key || (key != null && key.equals(k)))) {
                ...
                while(node.next != null){
                    if(node.next.hash == hash && ((k = node.next.key) == key || (key != null && key.equals(k)))){
                        return node.next;
                    }
                    else{
                        node = node.next;
    ...
    /**
     * Inserts the value using the Coalesced hashing insert algorithm.
     *
